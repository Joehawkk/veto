  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 mt-3 flex-wrap">
        <span className={`text-xs font-bold px-2.5 py-1 rounded-full border ${v.bg} ${v.text}`}>
          {v.icon} {v.label}
        </span>
      ...
      {/* Timer badge */}
      {hasTimer && (
        <div className={`mt-3 text-xs px-3 py-2 rounded-xl flex items-center gap-2 ${expired ? 'bg-secondary/10 border border-secondary/30 text-secondary' : 'bg-yellow-400/10 border border-yellow-400/30 text-yellow-400'}`}>
          <span>⏰</span>
      ...
      {isPending && (
        <div className="flex gap-2 mt-4" onClick={(e) => e.stopPropagation()}>
          ...
          <button onClick={() => onOutcome(entry.id, 'bought')}
            className="flex-1 bg-border text-muted text-sm font-bold py-2.5 rounded-xl hover:text-white active:scale-95 transition-all">
            🛒 Купил
          </button>
        </div>
      )}
  ...
  return (
    <div className="min-h-screen bg-bg">
      <header className="flex items-center gap-4 px-6 py-5 border-b border-border">
        <Link to="/" className="text-muted hover:text-white transition-colors">←</Link>
        <div>
          <h1 className="text-xl font-black text-white">История</h1>
      ...
      <main className="px-6 py-6 max-w-md mx-auto">
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          ...
          <div className="bg-card border border-border rounded-2xl p-3 text-center">
            <p className="text-white font-black text-lg">{stats.stopped}</p>
            <p className="text-muted text-[10px] mt-0.5">отказов</p>
          </div>
          <div className="bg-card border border-border rounded-2xl p-3 text-center">
            <p className="text-white font-black text-lg">{stats.bought}</p>
        ...
        {/* Pending alert */}
        {stats.pending > 0 && (
          <div className="bg-yellow-400/10 border border-yellow-400/30 rounded-2xl px-4 py-3 mb-5 flex items-center gap-3">
        ...
        {history.length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="text-white font-bold mb-1">История пуста</p>
            <p className="text-muted text-sm mb-6">Запусти первую проверку покупки</p>
            <Link to="/" className="inline-block bg-primary text-black font-black px-6 py-3 rounded-xl">
              Начать проверку
            </Link>
          </div>
        ) : (
